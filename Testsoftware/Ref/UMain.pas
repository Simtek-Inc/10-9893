...
//      Shift: TShiftState; X, Y: Integer);
  ...
  private
    ...
    procedure setPushbuttonSwitch(pb: TObject);
...
//    function  xlatCharTo14Segs(ch:char):cardinal;
...
//      C3RequestData[2]    := edtRateX10000.Tag;
//      C3RequestData[3]    := edtRateX01000.Tag;
//      C3RequestData[4]    := edtRateX00100.Tag;
//      C3RequestData[5]    := edtRateX00010.Tag;
//      C3RequestData[6]    := edtRateX00001.Tag;
//      C3RequestData[7]    := edtCabinAltX10000.Tag;
//      C3RequestData[8]    := edtCabinAltX01000.Tag;
//      C3RequestData[9]    := edtCabinAltX00100.Tag;
//      C3RequestData[10]   := edtCabinAltX00010.Tag;
//      C3RequestData[11]   := edtCabinAltX00001.Tag;
//      C3RequestData[12]   := edtDifPressX100.Tag;
//      C3RequestData[13]   := edtDifPressX010.Tag;
//      C3RequestData[14]   := edtDifPressX001.Tag;
//      C3RequestData[15]   := edtLDEConstAltX10000.Tag;
//      C3RequestData[16]   := edtLDEConstAltX01000.Tag;
//      C3RequestData[17]   := edtLDEConstAltX00100.Tag;
//      C3RequestData[18]   := edtLDEConstAltX00010.Tag;
//      C3RequestData[19]   := edtLDEConstAltX00001.Tag;
...
//      s        := s + Chr(C3RequestData[21]);     // 21                       //       (Bytes displayed in TX             //                                          //
//      s        := s + Chr(C3RequestData[22]);     // 22                       //       Commented out because there are    //                                          //
//      s        := s + Chr(C3RequestData[23]);     // 23                       //       only 19 digits for display. First  //                                          //
//      s        := s + Chr(C3RequestData[24]);     // 24                       //       is command byte.                   //                                          //
...
procedure TMainForm.setPushbuttonSwitch(pb: TObject);
begin
  case TShape(pb).Tag of
    $40 : TShape(pb).Pen.Color := clActive;
    else  TShape(pb).Pen.Color := clInactive;
  end;
end;
  ...
  with TShape(button) do
    begin
      if resetbit then pos := 0;
          case pos of
            $00: begin
                 TLabel(lblTop).Font.Color := clActive;      TShape(indexTop).Visible := True;     Tshape(indexBottom).Visible := False;
                 pos := pos + $01;
                 end;//end $00
...
begin
  ...
  //TEdit(Sender).Tag := Ord(TEdit(Sender).Text[1]);
...
begin
  if Tshape(decimalPoint).Tag = 1 then
    begin
      s := TEdit(Sender).Text;
      C3RequestData[TEdit(Sender).TabOrder+12] := ASCII27sEG(Ord(s[1]) or byte(10000000));
    end
...
function TMainForm.ByteToHex(num: byte): string;
var num1, num2 : byte;
    s : string;
begin
  num1 := num and $0F;
  num2 := (num and $F0) shr 4;
  case num2 of
   $1 : s := '1';
   $2 : s := '2';
   $3 : s := '3';
...
//      arraySwitches[1] := swSW01;
//      arraySwitches[2] := swSW02;
//      arraySwitches[3] := swSW03;
//      arraySwitches[4] := swSW04;
//      arraySwitches[5] := swSW05;
//      arraySwitches[6] := swSW06;
//      arraySwitches[7] := swSW07;
//
...
//      dLn1Ch1 := Chars14Segment[dScrollValue];
//      dLn1Ch2 := Chars14Segment[dScrollValue];
//      dLn1Ch3 := Chars14Segment[dScrollValue];
//      dLn1Ch4 := Chars14Segment[dScrollValue];
//      dLn1Ch5 := Chars14Segment[dScrollValue];
//      dLn1Ch6 := Chars14Segment[dScrollValue];
//      dLn1Ch7 := Chars14Segment[dScrollValue];
//      dLn1Ch8 := Chars14Segment[dScrollValue];
//      dLn2Ch1 := Chars7Segment[dScrollValue mod 27];
//      dLn2Ch2 := Chars7Segment[dScrollValue mod 27];
//      dLn2Ch3 := Chars7Segment[dScrollValue mod 27];
  ...
  if gRateC5.uc >= (tkbC5UpdateRate.Position) then
    begin
    if mm03s05.Checked and mm03s02.Checked then
      begin      if mm03s05s04.Checked then fC5Request := True;   end;
    gRateC5.uc := 0;
  ...
  if gRateC6.uc >= (tkbC6UpdateRate.Position) then
    begin
    if mm03s05.Checked and mm03s02.Checked then
      begin      if mm03s05s04.Checked then fC6Request := True;   end;
    gRateC6.uc := 0;
...
//         if (cal = giv) then
//          if (calculateChecksum(DPtr, C6Responselength) = ord(s[i+C6Responselength])) then
            begin
            inc(NumberOfResponse);
            responseStatus(DPtr);
...
begin
  fImageRedraw := False;
//  setPushbuttonSwitch(shpIndicatorD0);                                        // No light up pushbutton, but keeping for future reference
//  paintIndicator(C4RequestData[2], lblIndicatorD0);                           // No indicators, but keeping for future reference
...
begin                                                                           //
  if((pl and TControl(showing).Tag) = 0) then                                   //
    TLabel(showing).Font.Color := clBlack                                       //
  else                                                                          //
    TLabel(showing).Font.Color := clLime;                                       //
//fImageRedraw := True;                                                         //
end;
