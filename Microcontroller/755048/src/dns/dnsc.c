...
void DNSC_Event(U8_T id, U8_T event)
{
	...
	{
#if (UART0_ENABLE)
//		PRINTD(DEBUG_MSG,  ("Other event (id:%bx, event:%bx)\n\r", id, event));
#endif
	}
...
void dnsc_Send(U8_T InterUdpId, S8_T *pName, U32_T dnsIp)
{
	...
	{	
		...
		i = sprintf(strIp, "%du.", ((dnsIp >> 24) & 0x000000FF));
		i += sprintf(strIp + i, "%du.", ((dnsIp >> 16) & 0x000000FF));
		i += sprintf(strIp + i, "%du.", ((dnsIp >> 8) & 0x000000FF));
		sprintf(strIp + i, "%du", (dnsIp & 0x000000FF));
